        ...
        match pdfium.load_pdf_from_file(path.to_str().unwrap_or_default(), None) {
            Ok(doc) => {
                ...
                for (index, page) in doc.pages().iter().enumerate() {
                    ...
                    for link in page.links().iter() {
                        if let Ok(rect) = link.rect() {
                            ...
                            if let Some(action) = link.action() {
                                ...
                                } else if let Some(local_dest) =
                                    action.as_local_destination_action()
                                {
                    ...
                    let (image, thumbnail_image) = if let Ok(bitmap) =
                        ...
                        for pixel in rgba.pixels_mut() {
                            ...
                            if alpha < 255 {
                                ...
                                if alpha > 0 {
                                    ...
                                    pixel[0] = ((r - 255.0 * (1.0 - a_f32)) / a_f32)
                                        .clamp(0.0, 255.0)
                                        as u8;
                                    pixel[1] = ((g - 255.0 * (1.0 - a_f32)) / a_f32)
                                        .clamp(0.0, 255.0)
                                        as u8;
                                    pixel[2] = ((b - 255.0 * (1.0 - a_f32)) / a_f32)
                                        .clamp(0.0, 255.0)
                                        as u8;
                                } else {
                                    pixel[0] = 0;
